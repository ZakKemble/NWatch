...
#elif defined(__AVR_ATmega328P__) || defined(__AVR_ATmega32__) || defined(__AVR_ATmega328__)
#define RAMSTART (0x100)
...
int main(void)
{
  ...
  while(1)
  {
		...
		if(ch == STK_GET_PARAMETER)
		{
			...
			verifySpace();
			if (which == 0x82) // Send optiboot version as "minor SW version"
		...
		else if(ch == STK_SET_DEVICE) // SET DEVICE is ignored
			getNch(20);
		else if(ch == STK_SET_DEVICE_EXT) // SET DEVICE EXT is ignored
			getNch(5);
		...
		else if(ch == STK_PROG_PAGE)
		{
			...
			//if(!memType && address < NRWWSTART)
			//	__boot_page_erase_short((uint16_t)(void*)address);

			// Moved the data buffering here so the code is less than 512B, but its slightly slower at programming flash
			buffData(length);
		...
		else if(ch == STK_READ_SIGN) /* Get device signature bytes  */
		{
		...
		else // This covers the response to commands like STK_ENTER_PROGMODE
			verifySpace();
...
static void buffData(uint8_t length)
{
	uint8_t* bufPtr = buff;
	do *bufPtr++ = getch();
	while (--length);
